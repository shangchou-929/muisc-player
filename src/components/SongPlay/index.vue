<template>
  <div class="song_play">
    <div class="top">
      <div>
        <span class="m-logo">
          <svg class="logosong" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 184 34">
            <g fill="#fff">
              <path
      ...
      <div class="m_wrap">
        <div class="disc" @tap="test">
          <div :class="flag? 'isturn':''" class="turn">
            <div class="rollwrap">
              <div class="m_img">
            ...
            <div class="so_bg"></div>
            <span class="play_icon" v-show="!flag">
              <svg
                t="1565937724361"
                class="icon"
              ...
              >
                <path
                  ...
                  p-id="2981"
                />
                <path d="M404.9 336.3v352.6l305.4-176.3z" fill="#ffffff" p-id="2982" />
              </svg>
            </span>
          </div>
        ...
        <div class="song">
          ...
          <div class="song_info">
            <h2 class="h2_info">
              ...
              <span class="song_author">{{song.author}}</span>
            </h2>
          </div>
          <!-- 歌词显示 -->
          <ul class="song_lrc">
            <li ref="song_lrc" v-for="(item, index) in songContent" :key="index">{{item.lrc}}</li>
          </ul>
          <span class="showall">
            查看完整歌词
    ...
    <audio class="audio" hidden ref="audio" controls :src="songurl"></audio>
  </div>
</template>
...
export default {
  ...
  created() {
    this.axios
      .get("http://localhost:3000/lyric?id=" + this.songId)
      .then(res => {
        if (res.statusText === "OK") {
          
          if (res.data.nolyric == true) {
            this.songContent.push({ lrc: "此处暂无歌词~" });
          } else {
    ...
    this.axios
      .get("http://localhost:3000/song/url?id=" + this.songId)
      .then(res => {
        if (res.statusText === "OK") {
  ...
  methods: {
      ...
      audio.addEventListener("timeupdate", () => {
        ...
        if (audio.ended == true) {
          ...
          for (let j = 0; j < this.i; j++) {
            this.$refs.song_lrc[j].style = `top: 90px;`;
          }
          this.$refs.song_lrc[0].style = `top:0; color: #fefefe;`;
          this.$refs.song_lrc[1].style = `top:45px;`;
          this.i = 0;
        }
      });
    },
    ...
    formatLrc(lyric) {
      ...
      for (let i = 0; i < lines.length; i++) {
        let time = lines[i].substr(1, 9);
        let a = parseInt(time.substr(0, 2));
        let b = parseInt(time.substr(3, 2));
        let sec = a * 60 + b;
        let content = lines[i].substr(11);
        let obj = {
          time: time,
          lrc: content,
          sec: sec,
      ...
      this.songContent = arr;
    },
    //监听事件
    Playing(ct, du) {
      let i = this.i;
      if (i < this.songContent.length) {
        if (ct <= this.songContent[0].sec) {
          this.$refs.song_lrc[i].style = `top:0; color: #fefefe;`;
        }
        if (ct > this.songContent[i].sec) {
          //当前的内容
          this.i++;
          this.songContent[i] = { ...this.songContent[i], flag: true };
          this.$refs.song_lrc[i].style = `top:0; color: #fefefe;`;
          this.$refs.song_lrc[i - 1].style = `top:-45px;`;
          this.$refs.song_lrc[i + 1].style = `top:45px;`;
        }
      }
    }
        ...
        .turn {
          ...
          .rollwrap {
            ...
            .m_img {
              ...
              border-radius: 50%;
              overflow: hidden;
              background: url(//s3.music.126.net/mobile-new/img/disc_default.png?7c9b3ad…=)
                no-repeat;
              background-size: contain;
          ...
          .lgour {
            ...
            bottom: 0;
            z-index: 3;
            transform: matrix(-0.978139, 0.207954, -0.207954, -0.978139, 0, 0);

            .light {
              background: url(//s3.music.126.net/mobile-new/img/disc_light.png?2bb24f3…=)
                no-repeat;
              background-size: contain;
              position: absolute;
              left: 0;
              ...
              bottom: 0;
              z-index: 3;
              background-image: url(//s3.music.126.net/mobile-new/img/disc_light-ip6.png?996fc8a…=);
            }
          }
